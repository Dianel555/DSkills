...
// When running in Git Bash, paths like "/jd/tenacity" get converted to "C:/Program Files/Git/jd/tenacity"
function fixMsysPath(inputPath) {
  if (!inputPath) return inputPath;

  // Detect MSYS path conversion pattern
...
async function getContext(libraryId, query) {
  try {
    const fixedLibraryId = fixMsysPath(libraryId);
    const result = await makeRequest('/context', {
      libraryId: fixedLibraryId,
      query,
      type: 'json'
    });
    return result;

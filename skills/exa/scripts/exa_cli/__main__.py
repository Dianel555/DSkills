...
def _add_global_options(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--debug", action="store_true", help="Enable debug logging to stderr (sets EXA_DEBUG=true)")
    parser.add_argument(
        "--max-retry-wait",
        type=int,
        default=None,
        help="Cap (seconds) for single retry wait and exponential backoff (default: 60, env: EXA_MAX_RETRY_WAIT)",
    )
    parser.add_argument(
        "--auth-scheme",
        choices=["x-api-key", "bearer"],
        default=None,
        help="Authentication scheme: x-api-key (default) or bearer (env: EXA_AUTH_SCHEME)",
    )
...
def _build_search_parser(sub) -> None:
    ...
    p.add_argument(
        "--num-results", "-n", type=_num_results_in_range, default=10, help="Number of results 1-100 (default: 10)"
    )


def _build_fetch_parser(sub) -> None:
    p = sub.add_parser("web_fetch_exa", help="Batch URL extraction via /contents")
    p.add_argument("--urls", action="append", required=True, help="URL to fetch (repeat flag for multiple URLs)")
    p.add_argument("--max-chars", type=_positive_int, default=3000, help="Max characters per page (default: 3000)")
...
def _build_advanced_parser(sub) -> None:
    ...
    p.add_argument("--type", choices=["auto", "fast", "instant"], default="auto", help="Search type (default: auto)")
    p.add_argument("--category", help="Category filter")
    p.add_argument("--num-results", "-n", type=_num_results_in_range, default=10)
    p.add_argument(
        "--include-domains", action="append", default=None, help="Domain to include (repeat flag for multiple)"
    )
    p.add_argument(
        "--exclude-domains", action="append", default=None, help="Domain to exclude (repeat flag for multiple)"
    )
    p.add_argument(
        "--include-text", action="append", default=None, help="Required text snippet (repeat flag for multiple)"
    )
    p.add_argument(
        "--exclude-text", action="append", default=None, help="Excluded text snippet (repeat flag for multiple)"
    )
    ...
    p.add_argument("--max-chars", type=_positive_int, help="Max characters for text (only effective with --text)")
    p.add_argument("--out", "-o", help="Write JSON to file instead of stdout")
